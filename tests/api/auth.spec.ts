...
let token1: string;
    ...
    test.describe('Admin - Auth tests', () => {
        test('Admin - Reset password successfully', async () => {
            ...
            let resetPwResponse1 = await auth.resetPassword(newResetPw1, token1, StaticVariables.emailAdmin_1, StaticVariables.passwordAdmin_1); 
        ...
        test('Admin - Change email successfully', async () => {
            ...
            let changeEmailResponse = await auth.changeEmail(newChangeEmail, token, changedEmail, StaticVariables.passwordAdmin_1); 
            ...
            let changeEmailResponse1 = await auth.changeEmail(newChangeEmail1, token1, StaticVariables.emailAdmin_1, StaticVariables.passwordAdmin_1); 
    ...
    test.describe('Manager - Auth tests', () => {
        test('Manager - Reset password successfully', async () => {
            ...
            // reset to old password
            const newResetPw1 = await request.newContext();
            let resetPwResponse1 = await auth.resetPassword(newResetPw1, token1, StaticVariables.emailManager, StaticVariables.passwordManager); 
            expect(resetPwResponse1.data.message).toBe('Password reset successfully');
            await newResetPw1.dispose();
        });
        ...
        test('Manager - Change email successfully', async () => {
            ...
            const newChangeEmail = await request.newContext();
            let changeEmailResponse = await auth.changeEmail(newChangeEmail, token, changedEmail, StaticVariables.passwordManager); 
            expect(changeEmailResponse.data.message).toBe('Email changed successfully');
            await newChangeEmail.dispose();
            ...
            const newChangeEmail1 = await request.newContext();
            let changeEmailResponse1 = await auth.changeEmail(newChangeEmail1, token1, StaticVariables.emailManager, StaticVariables.passwordManager); 
            expect(changeEmailResponse1.data.message).toBe('Email changed successfully');
            await newChangeEmail1.dispose();
        });
    })
    ...
    test.describe('Contributor - Auth tests', () => {
        test('Contributor - Reset password successfully', async () => {
            ...
            // reset to old password
            const newResetPw1 = await request.newContext();
            let resetPwResponse1 = await auth.resetPassword(newResetPw1, token1, StaticVariables.emailContributor, StaticVariables.passwordContributor); 
            expect(resetPwResponse1.data.message).toBe('Password reset successfully');
            await newResetPw1.dispose();
        });
        ...
        test('Contributor - Change email successfully', async () => {
            ...
            const newChangeEmail = await request.newContext();
            let changeEmailResponse = await auth.changeEmail(newChangeEmail, token, changedEmail, StaticVariables.passwordContributor); 
            expect(changeEmailResponse.data.message).toBe('Email changed successfully');
            await newChangeEmail.dispose();
            ...
            const newChangeEmail1 = await request.newContext();
            let changeEmailResponse1 = await auth.changeEmail(newChangeEmail1, token1, StaticVariables.emailContributor, StaticVariables.passwordContributor); 
            expect(changeEmailResponse1.data.message).toBe('Email changed successfully');
            await newChangeEmail1.dispose();
        });
    })
